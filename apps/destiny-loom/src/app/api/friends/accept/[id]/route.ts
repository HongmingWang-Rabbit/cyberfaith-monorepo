...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const res = await fetch(`${CORE_API_URL}/v1/friends/accept/${id}`, {
      method: "POST",
    ...
    console.error("Friends accept proxy error:", error);
    return withRateLimitHeaders(errorResponse("Failed to accept friend request", 500));
  }
}
